...
const selectedRows = (rows) => {
  state.selectedRows = rows.map((r) => r.slug);
};
...
<template>
  <account-layout :title="$t('menu.admin.setting.acl.roles')">
      ...
      >
        ...
        <div class="user-management-options">
          ...
          >
            <router-link
              class="text-white"
              href="/account/admin/settings/acl/roles/new"
            >
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item>
                  <app-icon :size="14" icon="download-cloud" />
                  {{ $t("shared.export") }}</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
      ...
      >
        ...
        <el-table-column prop="name" :label="$t('acl.roles.role_name')">
          <template #default="scope">
            <router-link
              :href="`/account/admin/settings/acl/roles/${scope.row.slug}/edit`"
            >
            ...
            <br />
            <span v-if="scope.row.isInternal" class="small text-muted">
          ...
          <template #default="scope">
            <el-dropdown
              ...
              :disabled="
                ...
                (scope.row.isInternal && !isRoot) ||
                state.loadingKey === scope.row.slug
              "
              @click="
                redirectTo(
                  `/account/admin/settings/acl/roles/${scope.row.slug}/edit`
                )
              "
            >
              {{ $t("shared.edit") }}
                ...
                <el-dropdown-menu>
                  ...
                  >
                    <el-popconfirm
                      ...
                      "
                      @confirm="
                        handleDeleteRole(scope.row.slug, scope.row.isInternal)
                      "
