...
<template>
  <li>
    <template v-if="!item.children">
      <router-link :href="item.url" :class="{ 'text-primary': $page.url.startsWith(item.url) }">
    ...
    <ul class="nav-sub nav-mega" v-if="item.children">
      <app-sidebar-item v-for="(sub, i) in item.children" :key="i" :item="sub" :subSlide="true"></app-sidebar-item>
    </ul>
  </li>
</template>
